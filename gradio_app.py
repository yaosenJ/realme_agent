...
def api_send_message(message: str, thread_id: str, token: str = None) -> str:
    """发送消息"""
    try:
        response = requests.post(
            f"{API_BASE_URL}/v1/chat/completions",
            json={"messages": [{"role": "user", "content": message}], "stream": False, "thread": thread_id},
            timeout=60
        )
...
def create_app():
    with gr.Blocks(title="Realme智能客服") as app:
            ...
            with gr.Column(scale=1, min_width=280, elem_classes="sidebar-panel"):
                ...
                with gr.Group(visible=False) as session_form:
                    ...
                    with history_container:
                        history_buttons = gr.State(value=[])  # 存储对话按钮信息
        ...
        def do_login(user, pwd, token, name):
            ...
            if result.get("success"):
                ...
                return (
                    new_token, user, create_thread_id(), None,
                    f"👤 **{user}**",
            ...
            return (
                token, name, create_thread_id(), None,
                "欢迎访问",
                gr.update(), gr.update(),
        ...
        def do_guest(token, name):
            return (
                token, "访客", create_thread_id(), None,
                "👤 **访客模式**",
                gr.update(visible=False), gr.update(visible=True),
                [], "✅ 已进入访客模式",
                gr.update(choices=[], value=None)
        ...
        def do_send(message, history, thread, token, conv_id):
            """发送消息"""
            if not message.strip():
                return history, thread, conv_id, "⚠️ 请输入消息"

            history = history + [{"role": "user", "content": message}]
            response = api_send_message(message, thread, token)
            history = history + [{"role": "assistant", "content": response}]

            return history, thread, conv_id, ""
        ...
        login_btn.click(
            fn=do_login,
            inputs=[login_user, login_pass, user_token, username_state],
            outputs=[user_token, username_state, thread_state, current_conv_id, user_display,
        ...
        guest_btn.click(
            fn=do_guest,
            inputs=[user_token, username_state],
            outputs=[user_token, username_state, thread_state, current_conv_id, user_display,
                     login_form, session_form, chatbot, status_msg, history_radio]
        )
        ...
        send_btn.click(
            fn=do_send,
            inputs=[msg_input, chatbot, thread_state, user_token, current_conv_id],
